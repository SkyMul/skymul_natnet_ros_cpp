...
{   
    ...
    {
        std::cout << "Exception 1 occurred" << e.what() << std::endl << std::flush;
        // std::cout << "pClient: " << pClient << std::endl << std::flush;
        std::cout << "data->iFrame: " << data->iFrame << std::endl << std::flush;
        std::cout << "data->nMarkerSets: " << data->nMarkerSets << std::endl << std::flush;
        std::cout << "data->fTimestamp: " << data->fTimestamp << std::endl << std::flush;
        std::cout << "data->CameraMidExposureTimestamp: " << data->CameraMidExposureTimestamp << std::endl << std::flush;
        std::cout << "data->TransmitTimestamp: " << data->TransmitTimestamp << std::endl << std::flush;
    }
    // last_callback = ros::Time::now();
}
...
int main( int argc, char **argv)
{
    ...
    while(true)
    {
        ...
        {
            std::cout << "Exception 2 occurred" << e.what() << std::endl << std::flush;
        }
        ...
        {
            std::cout << "Exception 3 occurred" << e.what() << std::endl << std::flush;
        }
    }
